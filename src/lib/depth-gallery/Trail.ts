...
export class Trail {
	...
	minDistance = 0.006; // Minimum movement before adding a point
	maxPoints = 220; // Maximum points kept in the trail
	curveTension = 0.5; // Curviness of Catmull-Rom spline
	curveSegments = 220; // Max tube segments along the curve
	radialSegments = 8; // Tube roundness (sides around)
	radiusHead = 0.012; // Radius near latest point
	radiusTail = 0.003; // Radius near oldest point
	pointSmoothing = 0.3; // Lerp smoothing when adding points
	maxTrimPerFrame = 4; // Max point removals per frame
	jumpResetDistance = 999; // Hard reset threshold for huge jumps
	...
	addPoint(position: THREE.Vector3) {
		...
		// Trim points if we have too many
