...
export class TrailController {
	...
	private trailHeadParticles = new TrailHeadParticles(); // Optional sparkle experiment near trail head.
	private hasBoundDebug = false;
	private gallery: Gallery;
	private debug: boolean | null;
	private trailHeadPosition = new THREE.Vector3(); // Reused head position vector
	...
	constructor(gallery: Gallery, debug: boolean | null = null) {
	...
	init(scene: THREE.Scene, camera: THREE.Camera) {
		...
		this.seedInitialPoints(camera as THREE.PerspectiveCamera);
	}
	...
	update(camera: THREE.Camera, scroll: Scroll, time: number) {
		if (!camera) return;
	...
	getProgress(camera: THREE.Camera, scroll: Scroll) {
